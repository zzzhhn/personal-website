...
- The original three blue, green, and amber scroll-linked trajectories remain the background composition. Their line treatment uses a multi-filament Strands material with glow `1.15`; they still draw from top to bottom as page progress advances and retract when scrolling upward. A continuous internal phase now carries core glints, filament separation, and halo energy down each fixed trajectory at speed `0.5`. Rendering is disabled on mobile, coarse pointers, and reduced-motion devices.
...
6. `StrandsBackground.astro` mounts one viewport-sized WebGL renderer, but the shader maps every fragment into document coordinates owned by the post-Hero wrapper. It preserves the original seven anchors for each of the three trajectories, the original scroll start/end range, and the 4 percent per-strand stagger. Time changes only the bounded local wave and the internal core, filament, and halo phases, so the composition flows without drifting away from its scroll-linked paths. The renderer caps DPR at `1.35`, renders at 60 fps only while scrolling and 30 fps while idle, pauses while the page is hidden, and is never initialized on mobile or reduced-motion devices.
...
| Performance | Scroll writes only clip-path, transform, and opacity in requestAnimationFrame; media is click-loaded; Strands animation remains entirely shader-driven in one viewport-sized canvas with a `1.35` DPR cap, 60/30 fps pacing, and no per-frame layout reads or document-sized render target. |
...
- Exactly three distinct blue, green, and amber Strands follow the former ribbon trajectories. They progressively draw from top to bottom, retract in reverse, remain spatially stable after Motion, and use glow `1.15`; no fixed centered horizontal band appears. While scroll position is stationary, visible filament and luminosity phases continue to travel downward without moving the anchor geometry.
