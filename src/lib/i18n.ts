...
export const EXPERIENCES = {
  en: [
    ...
    },
    {
      ...
      type: "internship",
      highlights: [
        "Built an automated evaluation-data pipeline for R&D agents, independently developing Python analysis tooling and strong-model labeling workflows that achieved 95%+ automated coverage",
        "Designed a three-level intent taxonomy spanning coding, knowledge, general-purpose tasks, and 15+ fine-grained scenarios; combined human-in-the-loop review with few-shot calibration to raise human-model agreement from 60% to 90%+",
        "Owned core-metric analysis for AI coding modes and reused the AIME evaluation workflow to shorten reporting turnaround by 40%",
      ],
  ...
  zh: [
    ...
    },
    {
      ...
      type: "internship",
      highlights: [
        "面向研发 Agent 建设自动化评测数据 pipeline，独立开发 Python 分析工具与强模型打标流程，实现 95% 以上自动化覆盖率",
        "设计覆盖代码、知识、通用任务及 15+ 细分场景的三级意图分类体系，结合 Human-in-the-loop 与 few-shot 校准，将人机一致率由 60% 提升至 90% 以上",
        "负责 AI 编程模式核心指标分析，并复用 AIME 评测工作流，将报告交付周期缩短 40%",
      ],
